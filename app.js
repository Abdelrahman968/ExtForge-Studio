...
// ════════════════════════════════════════════════════════════
// EXTENSION BUILDER — FREE LOCAL GENERATION (NO API)
// ════════════════════════════════════════════════════════════
...
async function downloadZip(){
  if(!Object.keys(genFiles).length){showToast('Generate an extension first!',true);return;}
  // Build a simple zip using JSZip if available, else fall back to downloadAll
  ...
  TEMPLATES.forEach(t=>{
    ...
    card.innerHTML=`
      ...
      <div style="margin-top:12px">
        <button class="bsm hi" style="width:100%;justify-content:center" onclick="loadTemplate(${TEMPLATES.indexOf(t)})">⚡ Use Template</button>
      </div>`;
...
function randomTheme(){
  ...
  const colors={
    'editor.background':`hsl(${hue1},20%,10%)`,
    'editor.foreground':'#d4d4d4',
    'tab.activeBorderTop':`hsl(${hue1},70%,55%)`,
    'tab.activeBackground':`hsl(${hue1},20%,14%)`,
    'tab.inactiveBackground':`hsl(${hue1},20%,10%)`,
    'activityBar.background':`hsl(${hue1},22%,13%)`,
    'sideBar.background':`hsl(${hue1},20%,10%)`,
    'statusBar.background':`hsl(${hue2},50%,28%)`,
    'statusBar.foreground':'#ffffff',
    'titleBar.activeBackground':`hsl(${hue1},20%,13%)`,
  };
